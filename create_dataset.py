...
# predict データ（2023年のデータ）
predict_data = modeling_data[modeling_data.index.str.contains('2023')]
predict_data.to_csv('predict_data.csv', encoding='shift_jis')
# test データ(2022年のデータ)
test_data =  modeling_data[modeling_data.index.str.contains('2022')]
test_data.to_csv('prediction_rate_of_wins_test.csv', encoding='shift_jis')
# train　データ（2009〜2021年のデータ）
train_data = modeling_data.drop(predict_data.index, axis=0)
train_data = train_data.drop(test_data.index, axis=0)
train_data.to_csv('prediction_rate_of_wins_train.csv', encoding='shift_jis')
